...
def calculate_soguk_hava_deposu(request):
    ...
    try:
        ...
        # Emsal oranını al - PHASE 2 DİNAMİK EMSAL
        emsal_orani = request.data.get('emsal_orani')
        if emsal_orani is not None:
            emsal_orani = float(emsal_orani)
        
        result = soguk_hava_deposu.calculate_soguk_hava_deposu(arazi_alani, emsal_orani)
        
        if result['success']:
            logger.info(f"Soguk hava deposu calculation successful for alan: {arazi_alani}")
            return Response({
                'success': True,
                'message': result['message'],
                'data': {
                    'izin_durumu': 'TESİS YAPILABİLİR',
                    'arazi_alani': result['calculations']['arazi_alani'],
                    'emsal_orani': result['calculations']['emsal_orani'],
                    'maksimum_emsal': result['calculations']['maksimum_emsal'],
                    'soğuk_hava_deposu_alani': result['calculations']['soğuk_hava_deposu_alani'],
                    'idari_alan': result['calculations']['idari_alan'],
                    'teknik_alan': result['calculations']['teknik_alan'],
                    'toplam_yapi_alani': result['calculations']['toplam_yapi_alani'],
                    'kalan_emsal': result['calculations']['kalan_emsal'],
                    'emsal_kullanim_orani': result['calculations']['emsal_kullanim_orani'],
                    'html_mesaj': f"""
                    <div class="calculation-result">
                        <h3 style="color: #2d5016;">❄️ Soğuk Hava Deposu Değerlendirme Sonucu</h3>
                        <div class="result-item"><strong>📏 Arazi Alanı:</strong> {result['calculations']['arazi_alani']:,.0f} m²</div>
                        <div class="result-item"><strong>📐 Emsal Oranı:</strong> {result['calculations']['emsal_orani']}</div>
                        <div class="result-item"><strong>🏗️ Maksimum Emsal Alanı:</strong> {result['calculations']['maksimum_emsal']:,.1f} m²</div>
                        <div class="result-item"><strong>❄️ Soğuk Hava Deposu Alanı:</strong> {result['calculations']['soğuk_hava_deposu_alani']:,.0f} m²</div>
                        <div class="result-item"><strong>🏢 İdari Alan:</strong> {result['calculations']['idari_alan']:,.0f} m²</div>
                        <div class="result-item"><strong>🔧 Teknik Alan:</strong> {result['calculations']['teknik_alan']:,.0f} m²</div>
                        <div class="result-item"><strong>📊 Toplam Yapı Alanı:</strong> {result['calculations']['toplam_yapi_alani']:,.0f} m²</div>
                        <div class="result-item"><strong>📈 Emsal Kullanım Oranı:</strong> {result['calculations']['emsal_kullanim_orani']}</div>
                        <div class="result-item"><strong>📋 Kalan Emsal Hakkı:</strong> {result['calculations']['kalan_emsal']:,.1f} m²</div>
                        <div class="result-status success"><strong>✅ TESİS YAPILABİLİR</strong></div>
                    </div>
                    """
                }
            })
        else:
            logger.warning(f"Soguk hava deposu calculation failed: {result['message']}")
            return Response({
                'success': False,
                'message': result['message'],
                'data': {
                    ...
                    'html_mesaj': f"""
                    <div class="calculation-result">
                        <h3 style="color: #c41e3a;">🚫 Soğuk Hava Deposu Değerlendirme Sonucu</h3>
                        <div class="result-status error"><strong>❌ TESİS YAPILAMAZ</strong></div>
                        <div class="result-item"><strong>❗ Hata:</strong> {result['message']}</div>
                    </div>
                    """
                }
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        logger.error(f"Error in soguk hava deposu calculation: {str(e)}")
        return Response({
            'success': False,
...
def calculate_tarimsal_amacli_depo(request):
    ...
    try:
        logger.info(f"Tarimsal amacli depo calculation request: {request.data}")
        
        # Frontend'den gelen parametreleri al (alan_m2 veya alan)
        alan = request.data.get('alan_m2') or request.data.get('alan')
        emsal_orani = request.data.get('emsal_orani')  # Dinamik emsal oranı
        
        if alan is None:
            return Response({
                'success': False,
                'message': 'Alan parametresi gereklidir',
                'data': None
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({
                'success': False,
                'message': 'Alan geçerli bir sayı olmalıdır',
                'data': None
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        # Dinamik emsal oranı ile hesaplama yap
        result = tarimsal_amacli_depo.calculate_tarimsal_amacli_depo(alan, emsal_orani=emsal_orani)
        
        # Direkt result döndür (diğer yapılar gibi)
        logger.info(f"Tarimsal amacli depo calculation result: {result}")
        return Response(result)
    
    except Exception as e:
        logger.error(f"Error in tarimsal amacli depo calculation: {str(e)}")
        return Response({
            'success': False,
            'message': f'Hesaplama sırasında hata oluştu: {str(e)}',
            'data': None
